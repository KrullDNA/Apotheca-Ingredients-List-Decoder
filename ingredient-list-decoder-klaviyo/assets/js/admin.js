/**
 * The Klaviyo test-connection button.
 *
...
( function ( $ ) {
	...
	$( function () {
		$( '#ildk-test' ).on( 'click', function ( event ) {
			event.preventDefault();

			var status = $( '#ildk-test-status' );
			status.css( 'color', '' ).text( cfg.testing || '' );

			$.post( cfg.ajaxUrl, {
				action: cfg.action,
				nonce: cfg.nonce,
				api_key: $( '[name="ild_settings[klaviyo_api_key]"]' ).val(),
				list_id: $( '[name="ild_settings[klaviyo_list_id]"]' ).val()
